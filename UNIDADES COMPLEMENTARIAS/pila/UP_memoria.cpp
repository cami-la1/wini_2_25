...
String pilaM::ValidarSintaxis(const String& expr, TColor FormColor, TCanvas* Canvas) {
    String error = "";

    int A = 0;
    int B = Cm->Espacio_ocupado() - 1;
    if (B < 0) B = 0;
    ...
    Cm->ImprimirP_DE_A_A_B(FormColor, Canvas, x, y, A, B);

    if (MessageBox(NULL, L"Validando sintaxis. ¿Desea continuar?", L"Validación", MB_OKCANCEL) != IDOK) {
...
String pilaM::InfijaToPostfija2(const String& infija, TColor FormColor, TCanvas* Canvas) {
    ...
    if (MessageBox(NULL, L"Convirtiendo infija a postfija. ¿Desea continuar?", L"Conversión", MB_OKCANCEL) != IDOK) {
        return ""; // Cancelado por usuario
    }
...
float pilaM::EvaluarPostfija2(const String& expPostfija, TColor FormColor, TCanvas* Canvas) {
	...
	if (MessageBox(NULL, L"Evaluando expresión postfija. ¿Desea continuar?", L"Evaluación", MB_OKCANCEL) != IDOK) {
		return 0;
	}
