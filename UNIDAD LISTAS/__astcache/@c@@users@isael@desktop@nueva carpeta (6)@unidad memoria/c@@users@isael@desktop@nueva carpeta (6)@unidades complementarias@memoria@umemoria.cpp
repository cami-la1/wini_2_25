...
void CSmemoria::ImprimirP_DE_A_A_B(TColor FormColor, TCanvas *Canvas, int posX, int posY, int A, int B) {
	if (A <= B) {
		...
		Canvas->Font->Size = 8;
		// DIBUJAMOS CABECERA
	   // Pintado(posX, posY, "estado", FormColor, Canvas); // Nueva columna como la primera
		posX += TamanoCeldaX;
        Pintado(posX, posY, "dir", FormColor, Canvas);
        posX += TamanoCeldaX;
		Pintado(posX, posY, "dato", FormColor, Canvas);
        ...
        posY += TamanoCeldaY;
		Canvas->Font->Size = 7;
        // CICLO PARA LEER EL VECTOR SIMULADO DE MEMORIA
        for (int i = A; i <= B; i++) {
            if (mem[i].link != libre) {
                Canvas->Pen->Color = clBlack;
				// Columna "estado"   // si esta libre o ocupado
			  //	String estado = dir_libre(i) ? "L" : "O";
			 //	Canvas->Brush->Color = FormColor;
			  //  Canvas->Rectangle(posX, posY, posX + TamanoCeldaX, posY + TamanoCeldaY);
			   ...
			   //	posX += TamanoCeldaX;
				...
				Canvas->TextOutW(posX + 3, posY + 3, IntToStr(mem[i].link));
				posX += TamanoCeldaX;
				// Reiniciar la posición X y avanzar en Y
				posX = auxX;
				posY += TamanoCeldaY;
                Canvas->Brush->Color = FormColor;
            }
        }
        // Mostrar el mensaje de la posición libre al final
        String as = "Libre: " + IntToStr(libre);
        Pintado(posX, posY + 10, as, FormColor, Canvas);
	} else {
        ShowMessage("A tiene que ser mayor que B");
